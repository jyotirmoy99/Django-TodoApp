    ...
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            ...
            try:
                ...
                return render(request, 'todoApp/signupuser.html', {'form': UserCreationForm(), 'error': 'username has already been taken. Please choose a new username'})
        else:
...
def viewtodo(request, todo_pk):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, 'todoApp/viewtodo.html', {'todo': todo, 'form': form, 'error': 'Bad Data. Try Again'})
...
def completedtodos(request):
    todos = Todo.objects.filter(user=request.user, date_completed__isnull=False).order_by(
        '-date_completed')
